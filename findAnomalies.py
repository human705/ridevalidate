...
if len(sys.argv) != 3: sys.exit("2 command line argument expected, GPX or json activity file name and max slope %")
   ...
   for track in gpx.tracks:
      ...
      for segment in track.segments:
         for point in segment.points:
            if i == 0: start_time = point.time
            delta_time = point.time - start_time
            secs = delta_time.total_seconds()
            log_pts.append({"LAT": point.latitude, "LON": point.longitude, "ALT": point.elevation, "TIME": point.time, "SLOPE": 0.0, "SECS": secs})
            i += 1
...
#        dist = math.acos(1 if dist > 1 else dist) * 6371000
        ...
        dist = geoDistance(log_pts[pt-1]['LAT'], log_pts[pt-1]['LON'], log_pts[pt]['LAT'], log_pts[pt]['LON'])
        xyz1 = geolocation2xyz(log_pts[pt-1]['LAT'], log_pts[pt-1]['LON'], log_pts[pt-1]['ALT'])
        xyz2 = geolocation2xyz(log_pts[pt]['LAT'], log_pts[pt]['LON'], log_pts[pt]['ALT'])
        dist3D = math.sqrt((xyz2[0] - xyz1[0])**2 + (xyz2[1] - xyz1[1])**2 + (xyz2[2] - xyz1[2])**2)
        tot_dist3D += dist3D
        if dist == 0:
            bad_pts += 1
            print("Dist= 0 @ pt " + str(pt) + " SECS " + str(log_pts[pt]['SECS']) + " dEle= " + str(log_pts[pt]['ALT'] - log_pts[pt-1]['ALT']) + " Ele-1= " + str(log_pts[pt-1]['ALT']) + " Ele= " + str(log_pts[pt]['ALT']))
        else:
 ...
 #       dist_next = math.acos(1 if dist_next > 1 else dist_next) * 6371000
        dist_next = geoDistance(log_pts[pt]['LAT'], log_pts[pt]['LON'], log_pts[pt+1]['LAT'], log_pts[pt+1]['LON'])
        if dist_next != 0:
            slope_next = (log_pts[pt+1]['ALT'] - log_pts[pt]['ALT']) / dist_next * 100
            delta_elev = log_pts[pt]['ALT'] - (log_pts[pt-1]['ALT'] + dist / (dist + dist_next) * (log_pts[pt+1]['ALT'] - log_pts[pt-1]['ALT']))
    ...
    if abs(slope) > maxslope:
        if filetype == 'JSON':
            print("Slope prev/next=%6.2f /%6.2f, SLOPE(frm json))=%6.2f Dist prev/next=%6.2f /%6.2f, Delta E=%6.3f @pt %5d and SECS %s" % (slope, slope_next, log_pts[pt]['SLOPE'], dist, dist_next, delta_elev, pt, log_pts[pt]['SECS']))
        else:
            print("Slope prev/next=%6.2f /%6.2f, Dist prev/next=%6.2f /%6.2f, Delta E=%6.3f @pt %5d and SECS %s, time %s" % (slope, slope_next,  dist, dist_next, delta_elev, pt, log_pts[pt]['SECS'], log_pts[pt]['TIME']))
    ctr += 1

print("Bad pts " + str(bad_pts))
print("Max delta E: %6.2f at point %i" % (maxDE, maxDEpt))   
print("Number of activity points: %i, SECS at last point: %i" % (len(log_pts), log_pts[pt]['SECS']))
print()
print("Total distance on-the-flat: %6.1f" % (tot_dist))
print("Total distance on-the-slope: %6.1f, %% extra distance %2.1f" % (tot_dist3D, (tot_dist3D - tot_dist)/tot_dist*100))
